...
struct recover_str{
  ...
  #include "bxalloc.hpp"
  inline void* pm_malloc(size_t s) { return BXP_malloc(s); }
  inline void pm_free(void* p) { BXP_free(p); }
  inline void* pm_realloc(void* ptr, size_t new_size) { return BXP_realloc(ptr, new_size); }
  inline void* pm_calloc(size_t num, size_t size) { return BXP_calloc(num, size); }
  inline int pm_init() { printf("bxalloc\n");return BXP_init("bxalloc", REGION_SIZE); }
  inline void pm_close() { BXP_close(); }
  inline void pm_recover() { BXP_recover(); }
  template<class T>
  inline T* pm_get_root(unsigned int i){
    return BXP_get_root<T>(i);
  }
  inline void pm_set_root(void* ptr, unsigned int i) { BXP_set_root(ptr, i); }
  inline void pm_output(int i, size_t count, std::vector<double> objsize){
    fragment_array[0] = new _Float64 [160]; 
    fragment_array[1] = new _Float64 [160];
    memset(fragment_array[0], 0, sizeof(_Float64) * 40);
    memset(fragment_array[1], 0, sizeof(_Float64) * 40);
    BXP_output(40, count, fragment_array);
    _Float64 total_alloc=0;
    for(int j=0; j<160; j++)
    {
      total_alloc += fragment_array[i][j] * 65536;
    }
    printf("alloc bytes is %13f, %13f\n", (_Float64)count, total_alloc);
    std::cout<<"fragment rate "<<i<<" is "<< (_Float64)count / total_alloc<<std::endl;
    int mm=((_Float64)count / total_alloc)*1000000;
    std::cout<<"used rate "<<i<<" is "<< mm<<std::endl;
  }; 
  ...
  inline  void pm_output(int i, size_t count, std::vector<double> objsize){
    ...
    printf("alloc bytes is %13f, %13f\n", (_Float64)count, total_alloc);
    cout<<"fragment rate "<<i<<" is "<< (_Float64)count / total_alloc<<endl;
    int mm=((_Float64)count / total_alloc)*1000000;
    std::cout<<"used rate "<<i<<" is "<< mm<<std::endl;
  };
#elif defined(NVMMALLOC)
  #include <nvm_malloc.h>
  #define HEAP_FILE "/mnt/pmem/"
  inline void* pm_malloc(size_t sz){
    void *ret=nvm_reserve(sz);
    nvm_activate(ret,nullptr,nullptr,nullptr,nullptr);
    return ret;
  }
  ...
  inline int pm_init() { printf("pmdk\n");return 0; }
