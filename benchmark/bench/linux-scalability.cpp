...
static unsigned long size2 = 512;
...
main (int argc, char *argv[])
{
 ...
 // pthread_t thread[MAX_THREADS];
  HL::Fred * threads = new HL::Fred[MAX_THREADS];
  ...
  printf ("Object size: %lu, Iterations: %lu, Threads: %d\n",
	  size2, iteration_count, thread_count);
...
run_test (void * arg)
{
  ...
  register unsigned long request_size = size2;
  ...
  {
    void ** buf = (void **) pm_malloc(sizeof(void *) * total_iterations);
    
    for (i = 0; i < total_iterations; i++)
      {
	buf[i] = pm_malloc (request_size);
      }
