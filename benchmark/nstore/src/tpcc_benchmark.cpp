...
table* tpcc_benchmark::create_customer() {
  ...
  customer_do_new_order_schema = new ((schema*) pmalloc(sizeof(schema))) schema(cols);
...
table* tpcc_benchmark::create_stock() {
  ...
  stock_table_do_stock_level_schema = new ((schema*) pmalloc(sizeof(schema))) schema(cols);
...
void tpcc_benchmark::do_delivery(engine* ee) {
  ...
  for (d_itr = 0; d_itr < districts_per_warehouse; d_itr++) {
    ...
    // getNewOrder
    rec_ptr = new ((record*) pmalloc(sizeof(new_order_record))) new_order_record(new_order_table_schema, 0, d_itr, w_id);
    ...
    // getCId
    rec_ptr = new ((record*) pmalloc(sizeof(orders_record))) orders_record(orders_table_schema, o_id, 0, d_itr, w_id, 0, 0,
                                0, 1);
    ...
    rec_ptr = new ((record*) pmalloc(sizeof(customer_record))) customer_record(customer_table_schema, c_id, d_itr, w_id,
...
void tpcc_benchmark::do_new_order(engine* ee, bool finish) {
  ...
  // getWarehouseTaxRate
  rec_ptr = new ((record*) pmalloc(sizeof(warehouse_record))) warehouse_record(warehouse_table_schema, w_id, empty, empty,
                                 empty, 0, 0);
  ...
  // getDistrict
  rec_ptr = new ((record*) pmalloc(sizeof(district_record))) district_record(district_table_schema, d_id, w_id, empty, empty,
                                empty, 0, 0, 0);
  ...
  // getCustomer
  rec_ptr = new ((record*) pmalloc(sizeof(customer_record))) customer_record(customer_table_schema, c_id, d_id, w_id, empty,
  ...
  for (unsigned int i_itr = 0; i_itr < i_ids.size(); i_itr++) {
    ...
    // getItemInfo
    rec_ptr = new ((record*) pmalloc(sizeof(item_record))) item_record(item_table_schema, ol_i_id, 0, empty, 0);
    ...
    rec_ptr = new ((record*) pmalloc(sizeof(stock_record))) stock_record(stock_table_schema, ol_i_id, ol_supply_w_id, 0,
                               empty_v, 0, 0, 0, empty);
...
// getCustomerByLastName
    rec_ptr = new ((record*) pmalloc(sizeof(customer_record))) customer_record(customer_table_schema, 0, d_id, w_id, c_name,
                                  empty, empty, empty, 0, 0, 0, 0, 0, 0, 0,
                                  empty);
...
// getLastOrder
  rec_ptr = new ((record*) pmalloc(sizeof(orders_record))) orders_record(orders_table_schema, 0, c_id, d_itr, w_id, 0, 0,
                              0, 0);
...
// getOrderLines
  rec_ptr = new ((record*) pmalloc(sizeof(order_line_record))) order_line_record(order_line_table_schema, c_id, d_itr, w_id, 0,
                                  0, 0, 0, 0, 0, empty);
...
// getStockCount
  ...
  for (int o_id = min_o_id; o_id < d_next_o_id; o_id++) {
    ...
    rec_ptr = new ((record*) pmalloc(sizeof(stock_record))) stock_record(stock_table_schema, o_id, w_id, 0, empty_v, 0, 0,
                               0, empty);
