...
void *group_commit3(void *arg) {
  sp_engine *sp=(sp_engine *)arg;
  while (sp->ready) {

    if (!sp->read_only && sp->txn_ptr != NULL) {
      wrlock(&sp->gc_rwlock);
      assert(sp->bt->txn_commit(sp->txn_ptr) == BT_SUCCESS);
      sp->txn_ptr = sp->bt->txn_begin(0);
      assert(sp->txn_ptr);
      unlock(&sp->gc_rwlock);
    }

    std::this_thread::sleep_for(std::chrono::milliseconds(sp->conf.gc_interval));
  }
  return NULL;
}
...
sp_engine::sp_engine(const config& _conf, database* _db, bool _read_only,
  ...
  if (!read_only) {
   // gc = std::thread(&sp_engine::group_commit, this);
    if(conf.choose==3) 
    MAK_pthread_create(&gc,&attr,&group_commit3,(void*)this);
    else
    pthread_create(&gc,&attr,&group_commit3,(void*)this);
    ready = true;
  }
...
int sp_engine::update(const statement& st) {
 ...
 // string s((char*) val.data) ; 
  ...
  temp2[len]='\0';
  before_tuple = std::string(temp2);
 // before_tuple = std::string((char*) val.data);
 // cout<<val.size<<"   "<<before_tuple.size()<<":"<<before_tuple<<endl;
