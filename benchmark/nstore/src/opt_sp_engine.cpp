...
void *group_commit2(void *arg) {
  opt_sp_engine *opt_sp=(opt_sp_engine *)arg;
  while (opt_sp->ready) {

    if (!opt_sp->read_only && opt_sp->txn_ptr != NULL) {
      wrlock(&opt_sp->gc_rwlock);

      assert(opt_sp->bt->txn_commit(opt_sp->txn_ptr) == BT_SUCCESS);
      opt_sp->txn_ptr = opt_sp->bt->txn_begin(0);
      assert(opt_sp->txn_ptr);

      unlock(&opt_sp->gc_rwlock);
    }

    std::this_thread::sleep_for(std::chrono::milliseconds(opt_sp->conf.gc_interval));
  }
  return NULL;
}
...
opt_sp_engine::opt_sp_engine(const config& _conf, database* _db, bool _read_only,
  ...
  if (!read_only) {
    if(conf.choose==3) 
    MAK_pthread_create(&gc,&attr,&group_commit2,(void*)this);
    else
    pthread_create(&gc,&attr,&group_commit2,(void*)this);
...
opt_sp_engine::~opt_sp_engine() {
  ...
  if (!read_only) {
    ready = false;
    if(conf.choose==3) MAK_pthread_join(gc,NULL);
    else pthread_join(gc,NULL);
    //gc.join();
  }
...
int opt_sp_engine::insert(const statement& st) {
  ...
  val.data=(char*)pmalloc(sizeof(char)*(sizeof(record*) + 1));
...
int opt_sp_engine::update(const statement& st) {
 ...
 // update_val.data = new char[sizeof(record*) + 1];
  update_val.data=(char*)pmalloc(sizeof(char)*(sizeof(record*) + 1));
