...
namespace storage {
  ...
  extern pm_alloc pm1;
  extern bx_alloc pm2;
  extern makalu_alloc pm3;
  extern nvm_alloc pm4;
  extern pmdk_alloc pm5;
  extern void *pmp;
  ...
  static struct option opts[] = {
    {"pm_allocator",optional_argument,NULL,'P'},
  ...
  static void parse_arguments(int argc, char* argv[], config& state) {
    ...
    while (1) {
      int idx = 0;
      int c = getopt_long(argc, argv, "n:f:x:k:e:p:g:q:b:j:P:svwascmhludytzori", opts,
                          &idx);
...
fail:
	...
	exit(ret);
      ...
      case 'P':{
        ...
        state.choose=temp;
        if (temp==1) alloc=&pm1;
        else if(temp==2) alloc=&pm2;
        else if(temp==3) alloc=&pm3;
        else if(temp==4) alloc=&pm4;
        else if(temp==5) alloc=&pm5;
        else alloc=&pm1;
        break;}
      default:
        fprintf(stderr, "\nUnknown option: -%c-\n", c);
...
int main(int argc, char **argv) {
  ...
  if(temp==1||temp>5){
    libpm_min=LIBPM;
    libpm_max=LIBPM+PMSIZE;
  }
  else if(temp==2){
    libpm_min=(unsigned long long)storage::pmp;
    libpm_max=libpm_min+(unsigned long long)REGION_SIZE;
  }
  else if(temp==3){
    libpm_min=(unsigned long long)storage::pmp;
    libpm_max=libpm_min+(unsigned long long)REGION_SIZE;
    ...
    libpm_min=(unsigned long long)storage::pmp;
    libpm_max=libpm_min+(unsigned long long)(100ul*1024*1024*1024);
  }
  else if(temp==5){
    libpm_min=(unsigned long long)storage::pmp;
    libpm_max=libpm_min+(unsigned long long)REGION_SIZE;
 ...
 // storage::sp = (storage::static_info *) storage::pmemalloc_static_area();
  storage::sp = (storage::static_info *) pmalloc(sizeof(storage::static_info));
  pmemalloc_activate(storage::sp);
// Start
  ...
  //std::cerr << "TOTAL EPOCHS : " << get_tot_epoch_count()<<std::endl;
  /*
  int *p1,*p2,*p3,*p4,*p5;
  p1=(int*)pmalloc(sizeof(int));
  p2=(int*)pmalloc(sizeof(int));
  p3=(int*)pmalloc(sizeof(int));
