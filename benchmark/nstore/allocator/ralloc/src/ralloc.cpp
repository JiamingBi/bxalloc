...
int _RP_init(string filename,const char* _id, uint64_t size){
    filename_hh=filename;
    string filepath;
    string id(_id);
    // thread_num = thd_num;
    ...
    case SB_IDX:
        ...
        //start_addr=(void*)_rgs->regions_address[1];
        break;
    case META_IDX:
    ...
    /*
    for(int i=0;i<LAST_IDX;i++){
        printf("%d:%lld\n",i,(unsigned long long)_rgs->regions_address[i]);
    }*/
    start_addr=(void*)_rgs->regions_address[1];
    initialized = true;
    return (int)restart;
...
struct RallocHolder{
    int init_ret_val;
    RallocHolder(string filename,const char* _id, uint64_t size){
        init_ret_val = _RP_init(filename,_id,size);
    }
    ~RallocHolder(){
 ...
 */
int RP_init(string filename,const char* _id, uint64_t size){
   // filename_hh=filename;
    static RallocHolder _holder(filename,_id,size);
...
void* RP_set_root(void* ptr, uint64_t i){
    if(ralloc::initialized==false){
        RP_init(filename_hh,"no_explicit_init");
    }
