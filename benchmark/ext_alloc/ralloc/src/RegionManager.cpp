...
// 	char* ret = (char*) mmap((void*) 0, FILESIZE1,
...
void RegionManager::__map_persistent_region() {
  ...
  int fd;
  fd = open(HEAPFILE1.c_str(), O_RDWR | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);

  FD = fd;
  off_t offt = lseek(fd, FILESIZE1 - 1, SEEK_SET);
  assert(offt != -1);

  int result = write(fd, "", 1);
  assert(result != -1);

  void* addr = mmap(0, FILESIZE1, PROT_READ | PROT_WRITE, MMAP_FLAG, fd, 0);
  ...
  *(uint64_t*)((size_t)base_addr + 2 * sizeof(atomic_pptr<char>)) = FILESIZE1;
...
void RegionManager::__remap_persistent_region() {
  ...
  int fd;
  fd = open(HEAPFILE1.c_str(), O_RDWR, S_IRUSR | S_IWUSR);
...
void RegionManager::__map_transient_region() {
  ...
  void* addr = mmap(0, FILESIZE1, PROT_READ | PROT_WRITE,
...
void RegionManager::__destroy() {
  if (!exists_test(HEAPFILE1)) {
    std::cout << "File " << HEAPFILE1 << " doesn't exist!\n";
    return;
  }
  remove(HEAPFILE1.c_str());
  return;
}
