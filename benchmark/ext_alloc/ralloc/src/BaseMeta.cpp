...
template <class T, RegionIndex idx>
inline ptr_cnt<T> AtomicCrossPtrCnt<T, idx>::load(
...
void* BaseMeta::small_sb_alloc(size_t size) {
  ...
  while (true) {
    ...
    if (oldptr) {
      ...
      if (next >
          _rgs->regions[SB_IDX]->base_addr + _rgs->regions[SB_IDX]->FILESIZE1) {
        printf(
...
void* BaseMeta::do_malloc(size_t size) {
  ...
  if(cache->get_block_num() == 8180 && sc_idx == 1)
  {
    int stop_here = 1;
  }
...
void GarbageCollection::operator()() {
  ...
  while (curr_sb < sb_end) {
    ...
    while (curr_marked_blk != marked_blk.end() &&
           ((uint64_t)curr_sb >> SB_SHIFT) ==
               ((uint64_t)(*curr_marked_blk) >> SB_SHIFT)) {
    ...
    if (anchor.state == SB_EMPTY) {
      ...
      if (curr_desc->heap->sc_idx == 0) {
        ...
        if (anchor.count == 0) {
          ...
          assert((uint64_t)(free_blocks_head - curr_sb) %
                     curr_desc->block_size ==
                 0);
          anchor.avail =
