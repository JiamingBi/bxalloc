...
uint32_t BaseMeta::compute_idx(char* superblock, char* block, size_t sc_idx) {
   // SizeClassData* sc = get_sizeclass_by_idx(sc_idx);
    uint32_t const sc_block_size=myclasssize.indextosize[sc_idx];
    //uint32_t sc_block_size =uint32_t(ClassSize::getsizefromindex(sc_idx)) ;//sc->block_size;
    //(void)sc_block_size; // suppress unused var warning

    //assert(block >= superblock);
    //assert(block < superblock + SBSIZE);
    ...
    uint32_t idx = diff/sc_block_size;
    return idx;
    /*
    switch (sc_idx) {
...
void BaseMeta::fill_cache(size_t sc_idx, FreeList* cache) {
   ...
   // (void)sc;
   // assert(block_num > 0);
    //assert(block_num <= cache->Maxsize());
}

void BaseMeta::flush_cache(size_t sc_idx, FreeList* cache) {
    //ProcHeap* heap = &heaps[sc_idx];
    //SizeClassData* sc = get_sizeclass_by_idx(sc_idx);
    uint32_t const sb_size = SBSIZE;//sc->sb_size;
    uint32_t const block_size = myclasssize.indextosize[sc_idx];
    //uint32_t const block_size = ClassSize::getsizefromindex(sc_idx);//sc->block_size;
    // after CAS, desc might become empty and
    //  concurrently reused, so store maxcount
    uint32_t const maxcount = SBSIZE/block_size;//sc->get_block_num();
    ...
    while (cache->Size()>0) {
        char* head = cache->head_list();
        //printf("head:%p\n",head);
        char* tail = head;
        Descriptor* desc = desc_lookup(head);
       // char*a=desc->superblock;
        //printf("a:%p\n",a);
        ...
        cache->PopList(static_cast<char*>(*(pptr<char>*)tail), block_count);
...
void BaseMeta::malloc_from_partial(size_t sc_idx, FreeList* cache, size_t& block_num){
retry:
    ProcHeap* heap = &heaps[sc_idx];
    ...
    //assert(block_size==ClassSize::getsizefromindex(sc_idx));
    char* superblock = static_cast<char*>(desc->superblock);
    uint32_t max_flist_num=cache->Maxsize();
    if(oldanchor.count<=max_flist_num){
        do {
        ...
        cache->PushList(block, block_take);
        ...
        char *head=superblock + avail * block_size;
        char *cur=head;
        //if(cur<superblock||cur>(SBSIZE+superblock)) assert(0&&"head==nullptr");
        char *prev=cur;
        uint32_t num=0;
        //std::cout<<max_flist_num<<" "<<oldanchor.count<<std::endl;
        while(num<max_flist_num&&cur!=nullptr){
            prev = cur;
            cur = *(pptr<char>*)(cur);
            num++;
        }
        char *start=head;
        char *end=prev;
        *(pptr<char>*)end=nullptr;
        char *newhead=cur;
        //assert(newhead!=nullptr);
       
        uint32_t idx=compute_idx(superblock,newhead,sc_idx);
        Anchor newanchor;
        do{
            newanchor.avail=idx;
            newanchor.count=oldanchor.count-num;
            newanchor.state=SB_PARTIAL;
        }while(!desc->anchor.compare_exchange_weak(
                oldanchor, newanchor));
        FLUSH(&desc->anchor);
       ...
       // assert(desc->anchor.load().state == SB_PARTIAL);
        //heap_push_partial(desc);
        block_num += num;
        //assert(head!=nullptr);
        cache->PushList(head, block_num);
        //if(UNLIKELY(!cache->getinit()))
        //    cache->setinit();
        //else{
            if(max_flist_num*2<maxcount) cache->SetMaxSize(max_flist_num*2);
            else cache->SetMaxSize(maxcount);
        //} 
    }
}

void BaseMeta::malloc_from_newsb(size_t sc_idx, FreeList* cache, size_t& block_num) {
    ProcHeap* heap = &heaps[sc_idx];
    //SizeClassData* sc = get_sizeclass_by_idx(sc_idx);
    uint32_t const block_size=myclasssize.indextosize[sc_idx];
    //uint32_t const block_size = ClassSize::getsizefromindex(sc_idx);//sc->block_size;
    ...
    char* superblock = reinterpret_cast<char*>(small_sb_alloc(SBSIZE));
    //assert(superblock);
    ...
    //std::cout<<max_flist_num<<" "<<maxcount<<std::endl;
    if(max_flist_num>=maxcount){
        ...
        char* prev = cur;
        uint32_t num=0;
        //std::cout<<max_flist_num<<" "<<maxcount<<endl;
        while(num<max_flist_num&&cur!=nullptr){
            prev = cur;
            cur = *(pptr<char>*)(cur);
            //printf("num: %lu cur:%p\n",num,cur);
        ...
        *(pptr<char>*)end=nullptr;
        ...
        Anchor anchor;
        anchor.avail=idx;
        anchor.count=maxcount-num;
        anchor.state=SB_PARTIAL;
        desc->anchor.store(anchor);
        FLUSH(&desc);
        FLUSHFENCE;
        //assert(anchor.avail < maxcount );
        //assert(anchor.count < maxcount);
        // if state changes to SB_PARTIAL, desc must be added to partial list
        //assert(anchor.state == SB_PARTIAL);
        heap_push_partial(desc);
        block_num += max_flist_num;
        cache->PushList(superblock, max_flist_num);
       // if(UNLIKELY(!cache->getinit()))
        //    cache->setinit();
        //else{
            if(max_flist_num*2<maxcount) cache->SetMaxSize(max_flist_num*2);
...
inline void BaseMeta::organize_sb_list(void* start, uint64_t count){
    ...
    new (desc) Descriptor();
    uint64_t desc_index = (((uint64_t)desc)>>DESC_SHIFT) - (((uint64_t)_rgs->lookup(DESC_IDX))>>DESC_SHIFT);
    _queue->push(desc_index);
    for(uint64_t i = 1; i < count; i++){
      //  desc->next_free.store(desc+1);//pptr
        desc++;
        new (desc) Descriptor();
        desc_index = (((uint64_t)desc)>>DESC_SHIFT) - (((uint64_t)_rgs->lookup(DESC_IDX))>>DESC_SHIFT);
        _queue->push(desc_index);
    }
    /*
...
void* BaseMeta::small_sb_alloc(size_t size){
   ...
   // ptr_cnt<Descriptor> oldhead = avail_sb.load();
    while(true){
       ...
       auto desc_index=_queue->pop();
        if(desc_index.has_value()) {
                Descriptor* ret = reinterpret_cast<Descriptor*>(_rgs->lookup(DESC_IDX));
                ret+=desc_index.value();
                return reinterpret_cast<void*>(sb_lookup(ret));
        }
        else{
...
void* BaseMeta::do_malloc(size_t size){
    if (UNLIKELY(size > MAX_SZ)) {
        ...
        char *ptr=(char*)small_sb_alloc(sbs);
        assert(ptr);
        Descriptor* desc = desc_lookup(ptr);

        desc->heap = &heaps[NLIST];
        desc->block_size = sbs;
        desc->maxcount = 1;
        desc->superblock = ptr;
    ...
    //if(tsl_cache==nullptr) tsl_cache=new ThreadCache;
    // size class calculation
    //size_t sc_idx = get_sizeclass(size);
    size=ClassSize::RoundUp(size);
    //size_t sc_idx=ClassSize::Index(size);
    auto iterator = myclasssize.sizetoindex.find(size);
    size_t sc_idx=iterator->second;
    FreeList* cache = &tsl_cache._flist[sc_idx];
    // fill cache if needed
    if (UNLIKELY(cache->Size()==0))
        fill_cache(sc_idx, cache);
     //std::cout<<"mallc:"<<sc_idx<<" "<<cache->Size()<<" "<<cache->Maxsize()<<endl;
    return cache->Pop();
}
void BaseMeta::do_free(void* ptr){
  ...
  //  SizeClassData* sc = get_sizeclass_by_idx(sc_idx);
    ...
    // flush cache if need
    //std::cout<<"free:"<<sc_idx<<" "<<cache->Size()<<" "<<cache->Maxsize()<<endl;
    // if (UNLIKELY(cache->Size() >SBSIZE/ClassSize::getsizefromindex(sc_idx))){
...
// void BaseMeta::returnobject(char *start,size_t index){
//     while(start){
//         char *next_ptr=*(pptr<char>*)start;
//         Descriptor *desc=desc_lookup(start);
//         //assert(desc->block_size==ClassSize::getsizefromindex(index));
//         Anchor oldanchor=desc->anchor.load();
//         char *superblock=sb_lookup(desc);
//         uint32_t idx=compute_idx(superblock,start,index);
//         Anchor newanchor;
//         do {
//             // update anchor.avail
//             char* next = (char*)(superblock + oldanchor.avail * desc->block_size);
//             *(pptr<char>*)start = next;
//             uint32_t block_count=1;
//             newanchor = oldanchor;
//             newanchor.avail = idx;
//             // state updates
...
void bxalloc::public_flush_cache(){
    if(initialized) {
        for(int i=0;i<NLIST;i++){// sc 0 is reserved.
            base_md->flush_cache(i, &tsl_cache._flist[i]);
        }
    }
}
 ...
 * 
 * Description:
 *  Sequential stop-the-world garbage collection routine for bxalloc when dirty
 *  segment exists.
 */
void GarbageCollection::operator() () {
    ...
    for(int i = 0; i < MAX_ROOTS; i++) {
        if(base_md->roots[i]!=nullptr) {
            bxalloc::roots_filter_func[i](base_md->roots[i], *this);
        }
    }
    ...
    while(curr_sb < sb_end) {
        ...
        { 
            ...
            if(curr_desc->heap != nullptr &&
                ...
                if(curr_desc->heap->sc_idx == NLIST) {
