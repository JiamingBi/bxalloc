...
namespace bxalloc{
    ...
    // regions manager
    extern Regions* _rgs;//initialized when bxalloc constructs
    // flag indicating bxalloc is initialized or not.
    ...
    extern LockFreeQueue<uint64_t,80000> *_queue;
...
public:
    ...
    inline operator F*() const{// cast to absolute pointer
        if(UNLIKELY(is_null())){
            ...
            return reinterpret_cast<F*>(bxalloc::_rgs->translate(idx, off));
        }
    } 
    T& operator* ();
    T* operator-> ();
    ...
    inline CrossPtr& operator= (const F* p){
        ...
        off = bxalloc::_rgs->untranslate(idx, reinterpret_cast<char*>(tmp));
        return *this;
    }
...
public:
    ...
    inline void mark_func(T* ptr){
        ...
        if(UNLIKELY(!bxalloc::_rgs->in_range(SB_IDX, addr))) 
 ...
 *  Contains essential metadata for bxalloc, including:
 ...
 *  do_malloc() and do_free() are the real entry point of bxalloc's malloc and
 *  free routines.
 */
class BaseMeta {
public:
  ...
  //  RP_TRANSIENT AtomicCrossPtrCnt<Descriptor, DESC_IDX> avail_sb;
    ...
    RP_PERSIST ProcHeap heaps[NLIST+1];
    ...
    inline T* get_root(uint64_t i){
        ...
        assert(i<MAX_ROOTS);
        bxalloc::roots_filter_func[i] = [](const CrossPtr<char, SB_IDX>& cptr, GarbageCollection& gc){
...
private:
    ...
    // func on cache
    void fill_cache(size_t sc_idx, FreeList* cache);
public:
    // we need to call this function to flush TLS cache during exit
    void flush_cache(size_t sc_idx, FreeList* cache);
...
private:
    // helper func
    void returnobject(char *start,size_t index);
    ...
    void malloc_from_partial(size_t sc_idx, FreeList* cache, size_t& block_num);
    // fill cache by allocating a new sb in heap[sc_idx]
    void malloc_from_newsb(size_t sc_idx, FreeList* cache, size_t& block_num);
...
inline void GarbageCollection::filter_func(T* ptr){
    ...
    Descriptor* desc = bxalloc::base_md->desc_lookup((char*)ptr);
